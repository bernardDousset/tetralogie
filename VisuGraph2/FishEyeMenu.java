...
public class FishEyeMenu extends JMenu implements MenuListener {
	...
	fishEyeWindow.setLocation(location.x+45, location.y + getSize().height);
...
class FishEyeWindow extends JWindow implements MouseListener, MouseMotionListener,
    ...
    public boolean fish_selected=false;

    public String champ_req=" ";
...
public void mousePressed(MouseEvent e) { 
	try{ecrire(items[selectionIndex].menuItem.getLabel());}
	catch(IOException e1){};
	System.out.println(" Label = " + items[selectionIndex].menuItem.getLabel());

}

public void ecrire(String nom)throws IOException{
	champ_req=champ_req+" / "+items[selectionIndex].menuItem.getLabel();
	FileWriter classer = new FileWriter("nom");
	classer.write(items[selectionIndex].menuItem.getLabel());
	classer.close(); 
}
...
public void mouseClicked(MouseEvent e) { System.out.println("je suis la ");}
