...
public class Sommet implements Runnable{
	...
	/**
	 *  les variables stressDegree, stressStrength ... indiquent si on veut afficher la valeur numérique de la mesure ou non 
	 */
    ...
    /**
     * Variable booleen pour contrôler l'affichage des menus de calcul de centralite 
     */
	 ...
	 * menant à tous les noeuds du graphe
	 * 
	 * **/
...
//	
	...
	/*
	 * On reattribue des coordonnées à nos sommets dans le cas où on a activé
	 * les forces
	 */
	...
	public synchronized void epinglerDegree(Graphics g,  String mesure, double maxMesure){
		...
		if (this.visible) {
			...
			/* coordonnées d'où nous allons afficher les valeurs des centralités */
			int textPtX = this.getX()  ;
			int textPtY = this.getY()  - 10;
			if(mesure == "degre")
			valeurafficher =(double )Math.round(this.centralite_degre * 1000 )/1000;    // arrondissement à 3 chiffres prés
			
			if(mesure == "strength")
				valeurafficher =(double )Math.round(this.centralite_poids * 1000 )/1000;    // arrondissement à 3 chiffres prés
			if(mesure == "intermediarity")
				valeurafficher =(double )Math.round(this.betweennessCentrality * 1000 )/1000;    // arrondissement à 3 chiffres prés
			if(mesure == "proximity")
				valeurafficher =(double )Math.round(this.closenessCentrality* 1000 )/1000;    // arrondissement à 3 chiffres prés
			if(mesure == "CombinedDegre")
				valeurafficher =(double )Math.round(this.centralite_combine * 1000 )/1000;    // arrondissement à 3 chiffres prés
			if(mesure == "genProximity")
				valeurafficher =(double )Math.round(this.closenessGenCentrality* 1000 )/1000;    // arrondissement à 3 chiffres prés
			if(mesure == "genIntermediarity")
				valeurafficher =(double )Math.round(this.betweennessGenCentrality* 1000 )/1000;    // arrondissement à 3 chiffres prés
			if(mesure != ""){
...
//			}
	 ...
	 *            à l'instance de morphing, max correspond au maxmétrique
	 */
	...
	public synchronized void afficher(Graphics g, boolean Etiquette,
		...
		int periode = per;
		Graphics2D g2 = (Graphics2D) g;
		// si on a coché "afficher repere", on rend bien visible les reperes
		// pour chaque instance visualisée indépendement.
		...
		if (this.getVisible()) {
			...
			} else if (forme == 1) {                 // Taille = Metrique , on a deux représentation : barre et cercle suivant la valeur dy "type"
			...
			if (this.getVisible()) {
				...
				if (param == 1) {
					...
					for (int i = 1; i <= periode; i++) {
						...
						if (type == 1 && !clustering) {
							g2.setColor(Color.red);
							// la barre de l'histogramme correspondant à la
							// deuxième instance sera bleue
							if (i == 2)
								g2.setColor(Color.blue);
							// la barre de l'histogramme correspondant à la
							// deuxième instance sera blanche
							if (i == 3)
								g2.setColor(Color.white);
							// la barre de l'histogramme correspondant à la
							// deuxième instance sera orange
							if (i == 4)
